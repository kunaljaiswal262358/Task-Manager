...
function App() {
  ...
  return (
    <>
      <Navbar />
      <div className="md:container md:w-[35%] md:mx-auto min-h-[80vh] my-6 p-3 mx-5 bg-slate-200 rounded-xl">
        ...
        <label className='mx-2' htmlFor="show">See Finished</label>
        <div className="divider h-[1px] bg-gray-500"></div>
        <div >
          <div className="todos ">
              {todos.length === 0 && <p>No todo is here</p>}
              {todos.map(item=>{
                return (showFinished || !item.isCompleted) && <div key={item.id} className="todo flex gap-1 my-3 items-center">
                  <input id='item.id' checked={item.isCompleted} onChange={(e)=>{handleCheckbox(e ,item.id)}} type="checkbox" />
                  <div className={item.isCompleted ? "line-through w-1/2" : "w-1/2"}>{item.todo}</div>
                  <button className="edit mx-2  my-2 p-1 px-3 bg-violet-900 text-white rounded-2xl" onClick={(e)=>{handleEdit(e ,item.id)}} ><CiEdit /></button>
